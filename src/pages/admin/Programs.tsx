...
import { getAdminClient, type Program } from '@/lib/supabase';
import { Form, FormField, FormItem, FormLabel, FormControl, FormMessage } from '@/components/ui/form';
import { useForm } from 'react-hook-form';
import { FiEdit2, FiTrash2, FiPlus } from 'react-icons/fi';

const defaultForm = { title: '', description: '', duration: '', format: '', max_participants: 0, status: 'upcoming', featured: false, image_url: '' };
...
const Programs: React.FC = () => {
  ...
  const handleDelete = async () => {
    if (!deleting) return;
    const supabase = getAdminClient();
    await supabase.from('programs').delete().eq('id', deleting.id);
    setDeleting(null);
    fetchData();
  };
  ...
  const onSubmit = async (values: any) => {
    const supabase = getAdminClient();
    if (editing) {
      const { error } = await supabase.from('programs').update(values).eq('id', editing.id);
      if (error) {
    ...
    <div className="relative card-premium p-0 min-h-[70vh] animate-fade-in">
      <div className="flex items-center justify-between px-8 pt-8 pb-4 border-b border-border sticky top-0 z-10 bg-white/80 backdrop-blur-md rounded-t-2xl">
        ...
        <button
          ...
          onClick={openAdd}
        >
          <FiPlus className="text-lg" /> Add Program
        </button>
      </div>
      {loading ? (
        <div className="text-gray-400 text-center py-12">Loading...</div>
      ) : programs.length === 0 ? (
        <div className="text-gray-400 text-center py-12">No programs found.</div>
      ) : (
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {programs.map((p, i) => (
                ...
                >
                  <td className="py-3 px-4 font-medium text-gray-800 rounded-l-xl">{p.title}</td>
                  <td className="py-3 px-4 max-w-xs truncate">{p.description}</td>
                  <td className="py-3 px-4">{p.duration}</td>
                  <td className="py-3 px-4">{p.status.charAt(0).toUpperCase() + p.status.slice(1)}</td>
                  <td className="py-3 px-4">{p.featured ? 'Yes' : 'No'}</td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 animate-fade-in">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-premium p-8 w-full max-w-md relative animate-scale-in">
            ...
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">{editing ? <FiEdit2 /> : <FiPlus />} {editing ? 'Edit' : 'Add'} Program</h3>
            <Form {...form}>
      ...
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 animate-fade-in">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-premium p-8 w-full max-w-sm relative animate-scale-in">
            <h3 className="text-lg font-bold mb-4">Delete Program?</h3>
